...
<c:import url="../import/frameTop.jsp">
	<c:param name="progcd" value="NoticeList" />
</c:import>
...
<div id="ctm_wrap">
	...
	<form id="frmSearch" action="#" >
		<input type="hidden"  name="BO_ID"  id="BO_ID" value="B1"/>
...
--%>
	...
	function createGrid1(){
		...
		var colModel = [
			...
			{ name: 'BO_ID', width: 100, align: 'center' },
			{ name: 'BBS_NO', width: 100, align: 'center' },
			{ name: 'FILE_UID', hidden: true},			
			{ name: 'BBS_TITLE', width: 1000},
			{ name: 'REG_NM', width: 150, align: 'center' },
			{ name: 'REG_DT', width: 150, align: 'center' },
			{ name: 'BBS_VIEW_CNT', width: 100, align: 'center' },
			{ name: 'Del', width: 80, align: 'center', 
		  		formatter: function (cellval, options, rowObject) {	
		  			var se = "";
					if ("${loginVO.userId}" == "admin") {
						var bid = rowObject.BO_ID;
						var no = rowObject.BBS_NO;
						var fuid = "";						
						if (rowObject.FILE_UID != null && rowObject.FILE_UID != undefined) {
							fuid = rowObject.FILE_UID;
						}
						se = "<button class=\"btn btn-default\" type=\"button\" onClick=\"removeBBS('"+bid+"','"+no+"','"+fuid+"');\"><s:message code='button.delete'/></button>";	
					}			  			
					return se;
				}
			}
	...
	function removeBBS (bid, no, fuid) {
		...
		var url = "<c:url value="/common/BBSDelete.do"/>";		
		fn_ajax(url, false, params, function(data, xhr){		
			cSearch();
		});
	}
	...
	function cSearch(currentPage){
		...
		var url = "<c:url value="/common/selectBBSList.do"/>";
	...
	function grid1_ondblClickRow(rowid, iRow, iCol, e){
		...
		var param = {
			"BO_ID" : gridData["BO_ID"],
			"BBS_NO" : gridData["BBS_NO"]
		};
		var url = "<c:url value="/common/BBSView.do"/>";
		fn_pageMove(url, param);
	}
	
	function cUser1(){
		var BoardId = $("#BO_ID").val();
		var param = {
			"BO_ID" : BoardId
		};		
