  ...
  options: {
	...
	_create: function() {
		...
		this.container.find(".remove-all").click(function() {
			...
			var gid = fn_replaceAll($('.ui-multiselect').parent().parent().attr('id'), 'colchooser_', '');
			colmodel = $('#' + gid).jqGrid('getGridParam', 'colModel');
			colNames = $('#' + gid).jqGrid('getGridParam', 'colNames');
			...
			for (var i=0; i<colmodel.length; i++) {
				for (var j=0; j<options.length; j++) {
					if (options[j].label.trim() == colNames[i].trim() && colmodel[i].classes != 'gridnessasary') {
...
//
//			for (var i=0; i<colmodel.length; i++) {
//				if (item.text() == colNames[i] && colmodel[i].classes == 'gridnessasary') {
